 ...
 *   Sui wallets in the `ConnectButton`, AND are what LoginScreen.tsx's social
 *   buttons connect to via `dAppKit.connectWallet({ wallet })` — this is the
 *   only Enoki entry point in the app. The older `EnokiFlow` class (manual
 *   `createAuthorizationURL` + a redirect callback you complete yourself) is
 *   deprecated upstream and was removed from here: LoginScreen used it
 *   directly at one point, got a real id_token back from Google, and then
 *   silently dropped it because nothing called `EnokiFlow.handleAuthCallback`
 *   — `registerEnokiWallets` handles that whole exchange internally instead.
